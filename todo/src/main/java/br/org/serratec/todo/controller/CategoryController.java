...
public class CategoryController {
    ...
    @ApiResponses(value = { @ApiResponse(responseCode = "200", content = {
            ...
            @ApiResponse(responseCode = "505", description = "Exceção interna daaplicação") })
    @ResponseStatus(HttpStatus.CREATED)
    public ResponseEntity<CategoryDTO> inserir(@RequestBody CategoryDTO categoryDTO) {
        CategoryDTO a = categoryService.inserir(categoryDTO);
        if (a != null) {
    ...
    // @PutMapping("/{id}")
    // public ResponseEntity<Category> atualizar(@PathVariable Long id, Category
    // Category) {
